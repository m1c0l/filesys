	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if (od->od_ino) {
			...
			ok_so_far = filldir(dirent, od->od_name,
					...
					ftype);
			if (ok_so_far >= 0) {
				f_pos += OSPFS_DIRENTRY_SIZE; // f_pos++?
			}
		}
		else { // od->od_ino == 0, blank dir entry
			f_pos += OSPFS_DIRENTRY_SIZE; // f_pos++?
		}
	}
...
allocate_block(void)
{
	...
	for (free_block = 0; free_block <= nblocks; free_block++) {
		if (bitvector_test(bitmap, free_block)) { // if free
			bitvector_clear(bitmap, free_block); // 0 == allocated
			return free_block;
		}
	}
	return 0;
...
add_block(ospfs_inode_t *oi)
{
	...
	uint32_t dir_ind = direct_index(n),
		 indir_ind = indir_index(n);
...
remove_block(ospfs_inode_t *oi)
{
	...
	uint32_t *indir_ptr = 0,
		 *indir2_ptr = 0;

	uint32_t dir_ind = direct_index(index),
		 indir_ind = indir_index(index);
	...
	// remove from direct blocks
	if (indir_ind == -1) {
		direct = oi->oi_direct[index];
		free_block(direct);
		oi->oi_direct[index] = 0;
	}
	// remove from indirect block
	else if (indir_ind == 0) {
		indirect = oi->oi_indirect;
		if (indirect == 0) {
			return -EIO;
		}

		indir_ptr = (uint32_t*)ospfs_block(indirect);
		direct = indir_ptr[dir_ind];
	...
	// remove from indirect2 block
	else {
		indirect2 = oi->oi_indirect2;
		if (indirect2 == 0) {
			return -EIO;
		}

		indir2_ptr = (uint32_t*)ospfs_block(indirect2);
		indirect = indir2_ptr[indir_ind];
		if (indirect == 0) {
			return -EIO;
		}
